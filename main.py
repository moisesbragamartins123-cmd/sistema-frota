...
# --- CONFIGURAÇÃO E ESTÉTICA PREMIUM (FUSÃO) ---
st.set_page_config(page_title="PavControl - Gestão de Frota", layout="wide")
...
/* Títulos de campos e Formatação de Texto */
...
div.stButton > button:first-child {
    background-color: #1D9E75; color: white; border: none; border-radius: 8px; font-weight: 600; padding: 0.5rem 1.25rem; transition: background .2s;
}
...
[data-testid="metric-container"] {
    background: white; border: 1px solid #E2E8F0; border-radius: 10px; padding: .75rem 1rem;
}

/* Banners Elegantes de Saldo */
...
# --- FUNÇÃO GERADORA DE EXCEL COM TEMPLATE ---
...
def gerar_pdf_relatorio(df, tipo, titulo_esq, sub_esq, data_esq, dados_dir, titulo_tabela):
    ...
    pdf.set_xy(x_texto_esq, 12); pdf.set_font("Arial", 'B', 9)
    ...
    for chave, valor in dados_dir.items():
        if linha_atual % 2 == 0:
            pdf.set_x(127); pdf.cell(80, 5, f"{chave}: {valor}")
        else:
    ...
    if tipo == "SAIDAS":
        cols = [("DATA", 16), ("FICHA", 17), ("PLACA", 16), ("PREFIXO", 16), ("MÁQUINA/OPERADOR", 55), ("PRODUTO", 20), ("QTD (L)", 16), ("V. UNIT.", 16), ("TOTAL (R$)", 22), ("KM/HOR", 16), ("OBSERVAÇÃO", 67)]
    else:
        cols = [("DATA", 18), ("FICHA/NF", 25), ("FORNECEDOR/DISTRIBUIDORA", 65), ("TANQUE DESTINO", 45), ("PRODUTO", 25), ("QTD (L)", 22), ("V. UNIT.", 22), ("TOTAL (R$)", 25), ("OBSERVAÇÕES", 30)]
        
    for nome, largura in cols: pdf.cell(largura, 7, nome, border=1, align="C", fill=True)
    pdf.ln()

    pdf.set_font("Arial", '', 7)
    total_litros = 0; total_dinheiro = 0
    ...
    for _, r in df.iterrows():
        ...
        pdf.ln()
        total_litros += qtd; total_dinheiro += tot
...
if menu == "🏠 Painel Início":
    st.markdown("## 🏠 Painel de Controle da Obra")
    ...
    if not df_tanques.empty:
        ...
        cols_t = st.columns(min(len(df_tanques), 4))
    ...
    st.divider()
    st.subheader("Resumo de Gastos, Consumo e Produção")
    ...
    if not df_abast.empty:
        ...
        # Filtro da Produção no Mês
        t_prod_val = 0.0
        if not df_prod.empty:
            df_prod['data_valida'] = pd.to_datetime(df_prod['data'], errors='coerce')
            df_prod['Mes'] = df_prod['data_valida'].dt.strftime('%m/%Y')
            df_prod_filtrado = df_prod[df_prod['Mes'] == mes_selecionado] if mes_selecionado != "Todos" else df_prod
            t_prod_val = pd.to_numeric(df_prod_filtrado['quantidade'], errors='coerce').sum()
        ...
        m4.metric("Produção Total Lançada", f"{t_prod_val:,.1f}")
...
# --- PÁGINA: LANÇAR ABASTECIMENTO ---
    ...
    if df_v.empty: st.warning("⚠️ Cadastre veículos na aba 'Frota'.")
        ...
        st.info(f"⛽ Combustível Padrão: **{comb_padrao}** | ⏱️ Último Horímetro/Km: **{m_horimetro}**")
...
# --- PÁGINA: PRODUÇÃO DIÁRIA (NOVO MÓDULO) ---
elif menu == "🏗️ Produção Diária":
    st.markdown("## 🏗️ Produção Diária de Obra")
    
    with st.form("f_prod", clear_on_submit=True):
        st.markdown("#### Novo Registro de Produção")
        c1, c2, c3 = st.columns(3)
        data_pr = c1.date_input("Data", value=datetime.today())
        obra_pr = c2.text_input("Obra / Trecho")
        tipo_serv = c3.selectbox("Tipo de Serviço", [
            "Pavimentação Asfáltica","Base Granular","Sub-base","Terraplenagem",
            "Fresagem","Imprimação","Pintura de Ligação","Meio-fio e Sarjeta","Drenagem","Outro"
        ])
        
        c4, c5, c6 = st.columns(3)
        material = c4.text_input("Material (ex: CBUQ, Brita)")
        quantidade = c5.number_input("Quantidade", min_value=0.0, step=0.01)
        unidade = c6.selectbox("Unidade", ["t","m²","m³","m","un"])
        
        c7, c8, c9 = st.columns(3)
        estaca_ini = c7.text_input("Estaca Inicial (ex: 0+000)")
        estaca_fin = c8.text_input("Estaca Final (ex: 0+500)")
        responsavel= c9.text_input("Responsável / Equipe")
        
        c10, c11 = st.columns(2)
        status_pr = c10.selectbox("Status", ["Concluído","Em andamento","Paralisado"])
        obs_pr = c11.text_input("Observações")
        
        if st.form_submit_button("💾 Salvar Produção", use_container_width=True):
            if quantidade <= 0:
                st.error("⚠️ Preencha a quantidade com valor maior que zero.")
            elif not obra_pr:
                st.error("⚠️ Informe a Obra / Trecho.")
            else:
                try:
                    supabase.table("producao").insert({
                        "data": str(data_pr), "obra": obra_pr, "tipo_servico": tipo_serv, 
                        "material": material, "quantidade": quantidade, "unidade": unidade, 
                        "estaca_ini": estaca_ini, "estaca_fin": estaca_fin, 
                        "responsavel": responsavel, "status": status_pr, "observacao": obs_pr
                    }).execute()
    ...
    if not df_prod.empty:
        ...
        f_servico = c_f1.selectbox("Filtrar por Serviço", ["Todos"] + df_prod['tipo_servico'].dropna().unique().tolist())
        if f_servico != "Todos":
            df_prod = df_prod[df_prod['tipo_servico'] == f_servico]
            
        colunas_prod = [c for c in ["data","obra","tipo_servico","material","quantidade","unidade","estaca_ini","estaca_fin","responsavel","status","observacao"] if c in df_prod.columns]
        st.dataframe(df_prod[colunas_prod], use_container_width=True)
    else:
...
# --- PÁGINA: FORNECEDORES ---
    ...
    st.markdown("## 🏪 Cadastro de Postos e Distribuidoras")
    with st.form("ff"):
        c1, c2 = st.columns(2)
...
# --- PÁGINA: RELATÓRIOS E FECHAMENTOS ---
    ...
    st.markdown("## 📋 Central de Relatórios e Exportações")
    ...
    with t_geral:
        st.write("Tabela completa e limpa para exportação, com ordenação inteligente de horímetro.")
        if not df_s.empty:
            ...
            # Inteligência de Ordenação de Horímetro (Preservada!)
            ...
            n_cols = {'data':'Data', 'origem':'Origem', 'nome_tanque':'Tanque', 'numero_ficha':'Ficha', 'fornecedor':'Posto', 'prefixo':'Prefixo', 'classe':'Classe', 'placa':'Placa', 'motorista':'Operador', 'tipo_combustivel':'Produto', 'quantidade':'Litros', 'valor_unitario':'R$/L', 'total':'Total', 'horimetro':'KM/H', 'horas_trabalhadas':'Horas Trab.', 'consumo_l_h':'Consumo L/H', 'obra':'Obra', 'observacao':'Obs'}
